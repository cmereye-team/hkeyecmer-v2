...
const getDetail = async () => {
  ...
  const { data }: any = await useFetch(
    `https://hkcmereye.com/api.php/content/${pageId.value}`
  )
...
<template>
  <div class="healthInfoDetail">
    ...
    <div
      class="healthInfoDetail-content"
      v-loading="contentLoading"
      v-html="pageDetail.content"
    ></div>
    <div class="healthInfoDetail-btn healthInfoDetailBtnGA4">
      <div
        class="healthInfoDetail-btn-anim healthInfoDetailBtnGA4"
        @click="toWhatsapp"
      >
    ...
    <div class="healthInfoDetail-bottom">
      <div class="healthInfoDetail-bottom-info">
        <span class="ext_paperRecoFrom" v-if="pageDetail.ext_paperRecoFrom"
          >新聞來源︰{{ pageDetail.ext_paperRecoFrom }}</span
        >
        <span
          class="Bowtie"
          v-if="pageDetail.ext_paperRecoFrom === 'Bowtie'"
          @click="goLink('https://www.bowtie.com.hk')"
          >合作醫療夥伴︰Bowtie</span
        >
        <span class="author" v-if="pageDetail.author"
          >作者︰{{ pageDetail.author }}</span
        >
        <span
          class="source"
          v-if="pageDetail.source"
          @click="goLink(pageDetail.source)"
          >資料來源︰原文鏈接</span
        >
        <span v-if="pageDetail.visits">瀏覽次數︰{{ pageDetail.visits }}</span>
        <span v-if="pageDetail.update_time"
          >更新時間︰{{ pageDetail.update_time }}</span
        >
      </div>
...
.healthInfoDetail {
  ...
  &-btn {
    ...
    &-anim {
      ...
      transition: all 0.3s;
      &:hover {
        box-shadow: 0px 4px 10px rgba($color: #00dd8b, $alpha: 0.7);
        .healthInfoDetail-btn-in {
          span {
            text-shadow: 0px 0px 3px rgba($color: #fff, $alpha: 0.5);
          }
        }
      }
    }
